...
def impute_mask(
    net, x, mask, num_samples, dim, num_steps=50, device='cuda:0',
    progress=True,
):
    ...
    with torch.no_grad():
        ...
        for i, (t_cur, t_next) in tqdm(
            ...
            total=num_steps - 1, disable=not progress,
        ):
            if i < num_steps - 1:
...
class EDMLoss:
    ...
    def __call__(self, denoise_fn, data, mask=None):
        ...
        if mask is None:
            ...
            mask = ~mask.bool()
            loss = weight.unsqueeze(1) * ((D_yn[mask] - target[mask])) ** 2
        return loss
